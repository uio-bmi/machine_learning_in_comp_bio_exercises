...
def train_and_assess_logistic_regression(encoded_train, train_labels, encoded_test, test_labels, feature_names, result_path, C):
    ...
    plot_top_n_coefficients(logistic_regression.coef_.flatten(), 'logistic regression', feature_names, result_path, 10)
...
def print_dataset_info(train_dataset, test_dataset=None):
    ...
    print(f"{'Training dataset' if test_dataset is not None else 'Dataset'}: {train_dataset.shape[0]} examples, {sum(train_dataset['label'].values)} positive and "
...
def encode_kmer(k: int, data: pd.DataFrame, vectorizer, scaler, learn_model: bool, path):
    ...
    # kmer_enc = KmerEncoding(k=k, alphabet_encoding=bionumpy.encodings.DNAEncoding)
    # kmers = kmer_enc(bionumpy.as_encoded_array(data['sequence'].values.tolist(), target_encoding=bionumpy.encodings.DNAEncoding))
